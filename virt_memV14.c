...
//	This is the completed version of the virt_mem PROJECT with FIFO
//   Aarib Abdullah
    ...
    */
    ...
    if((found_frame = IsItInTLB(page)) == -1){
		if((found_frame = IsItInPageTable(logic_add, page)) == -1){	// when this is true, this handles the page fault
			...
			//fseek(fbinary , logic_add, SEEK_SET);
			...
			found_page = page ;		
			//printf("\nBACKING STORE PGFLT\t");
			int start_index = found_frame * FRAME_SIZE;
			...
			invalidate_Bit(found_page, found_frame);
