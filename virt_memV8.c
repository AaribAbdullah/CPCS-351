...
PGTBL.queueOutPoint++;
if(TLB.queueOutPoint >= PAGE_TABLE_ENTRIES) {PGTBL.queueOutPoint = 0;}
...
int main(int argc, const char * argv[]) {
  ...
  FILE* fbinary = fopen("BACKING_STORE.bin", "rb");
  ...
  while (frame < NUM_OF_FRAMES) {
    ...
    unsigned int found_page, found_frame;
     
    //if (page == 244) { printf("found 244\n"); 	}
    ...
    if((found_frame = IsItInTLB(page)) == -1){
		if((found_frame = IsItInPageTable(logic_add, page)) == -1){	// when this is true, this handles the page fault
			fseek(fbinary , (frame -1) * FRAME_SIZE, SEEK_SET);
			fread(buf, sizeof(char), 256, fbinary);
			
			found_frame = frame -1;
			found_page = page ;		
			//printf("\nBACKING STORE PGFLT\t");
			unsigned start_index = found_frame * FRAME_SIZE;
			for(unsigned int i = 0; i < FRAME_SIZE; i++){
					
					//Physical_addr[ start_index + i] = buf[i];
					
					Physical_addr[ start_index] = buf[i];
					start_index++;
					
					//printf("\t%5u\t%5u\t", Physical_addr[start_index + i], buf[i]);
					
			}	
			
			update_PGTBL(found_page, found_frame);
			update_TLB(found_page, found_frame);
  ...
  printf("\nSTATISTICS:\nPage-fault rate: %f \nTLB hit rate: %f \n\n",(((double)PGTBL.page_table_miss/frame)*100), (((double)TLB.tlb_hit/frame)*100));
  return 0;
}
